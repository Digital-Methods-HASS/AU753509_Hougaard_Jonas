...
# First we have the longitude and after that the latitude
...
#It almost doesn't change. Only the layout of the box in the top right corner is changed
...
l_DANmap <- leaflet() %>%   # assign the base location to an object
  setView(10.85089,55.2339089, zoom = 13)

esri <- grep("^Esri", providers, value = TRUE)

for (provider in esri) {
  l_DANmap <- l_DANmap %>% addProviderTiles(provider, group = provider)
}

DANmap_rigtig <- l_DANmap %>%
  addLayersControl(baseGroups = names(esri),
                   options = layersControlOptions(collapsed = FALSE)) %>%
  addMiniMap(tiles = esri[[1]], toggleDisplay = TRUE,
             position = "bottomright") %>%
...
saveWidget(DANmap_rigtig, "DANmap_rigtig.html", selfcontained = TRUE)
...
# Your brief answer
# Clustering combines multiple marks, and that makes it easy to see where most of the landmarks are centered.
# The map without clustering however, is good for seeing exactly where each mark is and what it is about 
...
# Solution
DANmap_cluster_and_classifications <- l_DANmap %>% 
  addLayersControl(baseGroups = names(esri),
                   options = layersControlOptions(collapsed = TRUE)) %>%
  addMiniMap(tiles = esri[[1]], toggleDisplay = TRUE,
             position = "bottomright") %>%
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Description, "<br>"
                           ,places$Notes
                           ,places$Type),
             clusterOptions = markerClusterOptions(),) %>% 
  addMeasure(
...
DANmap_cluster_and_classifications
