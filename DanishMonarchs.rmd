...
read_csv and read_csv2 are "tidyverse" functions. We now this because tidyverse uses the _

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
By running class() on the kings object we get the output "spec_tbl_df" "tbl_df" "tbl" "data.frame". The first is specification table data and its a tibble that tidyverse has created. The second and third are also tibble which tidyverse has created. The last is data frame and then it turns it into a vector

3. How many columns does the object have when created with these different functions?
There are 11 columns, which i can see by pressing view(kings)

4. Show the dataset so that we can see how R interprets each column
Here i press head(kings) to see how R interprets the columns
...
# YOUR CODE
Reign_duration %>%
  filter(as.numeric(Reign_duration) > mean(as.numeric(Reign_duration), na.rm = TRUE)) %>% 
  count(Reign_duration)
...
Reign_duration %>%
  mutate(Reign_duration = as.numeric(Reign_duration)) %>%
  arrange(desc(Reign_duration)) %>%
  slice(1:3) %>%
  mutate(Days = Reign_duration * 365)
...
Reign_duration %>%
  mutate(
    Reign_duration = as.numeric(Reign_duration),
    midyear = Era_start + (Era_finished - Era_start) / 2
  ) %>%
